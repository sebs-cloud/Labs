...
void writeDigitToLatch(int digit) {
	digitalWrite(octalLatchEnablePin,HIGH);
	for (int i = 0; i <= 6; i++) {

				digitalWrite(latchD0Pin + 1 ,octalLatchDigitArray[digit][i]);
			}
			digitalWrite(octalLatchEnablePin,LOW);
}
...
void writeDigitToDriver(byte digit) {
	digitalWrite(bcdDriverLatchEnablePin,HIGH);
	digitalWrite(driverAPin,bcdDriverDigitArray[digit][3]);
	digitalWrite(driverBPin,bcdDriverDigitArray[digit][2]);
	digitalWrite(driverCPin,bcdDriverDigitArray[digit][1]);
	digitalWrite(driverDPin,bcdDriverDigitArray[digit][0]);
	digitalWrite(bcdDriverLatchEnablePin,LOW);
...
void writeDigitToRegister(int digit) {
  shiftOut(serialInputPin, shiftRegisterClockPin, MSBFIRST, 25);
}
